...
export {
	...
	## A publishing connection should not see its own messages.
	## By default, a NATS consumer sees its own messages, but
	## for Zeek we do not want this behavior.
...
event Cluster::Backend::NATS::hello(name: string, id: string)
	...
	if ( ! n?$id )
		...
		# We raise node_down() here for the old instance, assuming
		# it crashed and never managed to send goobye.
